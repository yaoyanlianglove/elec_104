...
typedef struct STRU_104_Config_TypeDef_t
{
    ...
    uint8_t  controlTimeout;                /* 遥控超时 */
    uint32_t deviceAddr;                    /* 设备id */
    uint8_t  asduHeaderLength;              /* 头长度 */
...
typedef struct STRU_104_TypeDef_t
{
    ...
    uint8_t  rxOverFlag;                    /* 接收完完整的数据帧标志 */
    uint8_t  *tx;                           /* tx buffer发送数据 */
    ...
    uint8_t  initQPR;                       /* 初始化原因 */
    uint8_t  timerCloseFlag;                /* 定时器线程结束标志 */

    STRU_CMD_STAT_TypeDef *cmdStat;         /* 命令状态 */   
 
    STRU_ASDU_TypeDef *rep;                 /* ASDU请求 */
    STRU_ASDU_TypeDef *req;                 /* ASDU回复 */
